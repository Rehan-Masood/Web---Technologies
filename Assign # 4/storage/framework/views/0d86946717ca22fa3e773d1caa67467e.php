...
<head>
    ...
    <title><?php echo e($title ?? 'TaskFlow Premium'); ?></title>
...
<div class="app">
    <aside class="sidebar" id="sidebar">
        ...
        <nav class="nav">
            ...
            <a class="<?php echo e(request()->routeIs('tasks.index') ? 'active' : ''); ?>" href="<?php echo e(route('tasks.index')); ?>">✅ Tasks</a>
            <a class="<?php echo e(request()->routeIs('tasks.create') ? 'active' : ''); ?>" href="<?php echo e(route('tasks.create')); ?>">➕ Create Task</a>
            <a class="<?php echo e(request()->routeIs('kanban') ? 'active' : ''); ?>" href="<?php echo e(route('kanban')); ?>">🗂 Kanban Board</a>
            <a class="<?php echo e(request()->routeIs('calendar') ? 'active' : ''); ?>" href="<?php echo e(route('calendar')); ?>">📅 Calendar</a>
            <a class="<?php echo e(request()->routeIs('analytics') ? 'active' : ''); ?>" href="<?php echo e(route('analytics')); ?>">📈 Analytics</a>
            <a class="<?php echo e(request()->routeIs('settings') ? 'active' : ''); ?>" href="<?php echo e(route('settings')); ?>">⚙️ Settings</a>
        </nav>
        <div class="user-card">Logged in as <strong><?php echo e(auth()->user()->name); ?></strong><span><?php echo e(auth()->user()->email); ?></span></div>
    </aside>
    <main class="main">
        <header class="topbar">
            <button class="btn secondary mobile-toggle" onclick="document.getElementById('sidebar').classList.toggle('open')">☰</button>
            <input class="search" placeholder="Search tasks, projects..." onkeydown="if(event.key==='Enter'){location.href='<?php echo e(route('tasks.index')); ?>?search='+encodeURIComponent(this.value)}">
            <div class="actions">
                <div class="notify" id="notifyBox">
                    ...
                    <?php if(($notificationCount ?? 0) > 0): ?><span class="notify-badge"><?php echo e($notificationCount); ?></span><?php endif; ?>
                    <div class="notify-menu">
                        <h3 style="margin:0 0 12px">Notifications</h3>
                        <?php $__empty_1 = true; $__currentLoopData = ($notifications ?? collect()); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $note): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                            <div class="notify-item"><?php echo e($note); ?></div>
                        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); if ($__empty_1): ?>
                            <div class="notify-item">No new notifications.</div>
                        <?php endif; ?>
                    </div>
                </div>
                <a class="btn" href="<?php echo e(route('tasks.create')); ?>">+ New Task</a>
                <form method="POST" action="<?php echo e(route('logout')); ?>"><?php echo csrf_field(); ?><button class="btn secondary">Logout</button></form>
            </div>
        </header>
...
<?php /**PATH D:\XAMP\htdocs\todo-premium\resources\views/layouts/premium.blade.php ENDPATH**/ ?>