...
function downloadPNG() {
  ...
  if (!element) {
    showToast("CV preview not found. Please ensure the form is loaded.", "error");
  ...
  if (typeof html2canvas === 'undefined') {
    showToast("Image export library failed to load. Please refresh and try again.", "error");
  ...
  html2canvas(element, {
    ...
    ignoreElements: function(node) {
      // Ignore map and other non-essential elements
      if (node.id === 'leafletMap' || node.id === 'mapPlaceholder') {
        return true;
      }
      return false;
    }
    ...
    .then((canvas) => {
      ...
      link.download = `cv_${name.replace(/\s+/g, "_")}.png`;
...
function downloadJPEG() {
  ...
  showToast("Generating JPEG... Please wait.", "info");
  ...
  html2canvas(element, {
    ...
    .then((canvas) => {
      const link = document.createElement("a");
      // Use quality 0.95 for high-quality JPEG
      link.href = canvas.toDataURL("image/jpeg", 0.95);
      link.download = `cv_${name.replace(/\s+/g, "_")}.jpeg`;
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);
    ...
    .catch((error) => {
      ...
      showToast("Error exporting JPEG. Please try again.", "error");
    });
}
